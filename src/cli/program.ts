...
export interface ProgramSession {
  /** Broker used when a command gives no --broker flag. */
  readonly broker?: BrokerName;
  /** False when the session was started with --no-cache. */
  readonly cache?: boolean;
  /** True inside the REPL, which must not offer to start another REPL. */
  readonly interactive?: boolean;
}

/**
 * Build the complete huskly-cli command table.
 *
 * @remarks
 * The bin and the REPL both call this, so there is exactly one definition of
 * every command, option, default, and help text. A REPL that re-declared the
 * table drifted from the CLI, which is the bug this removes.
 *
 * Each call returns a fresh tree, because Commander stores parsed option
 * values on the command objects and a REPL parses many lines in one process.
 */
export function createProgram(session: ProgramSession = {}): Command {
  ...
  program
    ...
    .hook("preAction", () => {
      // A --no-cache session cannot be re-enabled by a later command.
      setCacheEnabled(session.cache !== false && program.opts<{ cache: boolean }>().cache);
    });
  ...
  function broker(override?: string, fallback: BrokerName = "schwab"): BrokerName {
    return chooseBroker(
      override,
      program.opts<{ broker?: string }>().broker,
  ...
  program
    ...
    .option("--json", "Emit a stable JSON DTO")
    .action(
      async (index: string, options: { sort?: string; frequency?: string; json?: boolean }) => {
        guardSchwab("movers");
